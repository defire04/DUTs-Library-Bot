...
def get_pages_from_search_query(search_string: str):
    ...
    search_result = SearchResult(book_list)
...
async def echo_message(msg: types.Message):
    ...
    for book in pages.get_page(0):
        books_strings.append("Название книги: " +
    ...
    await bot.send_message(msg.from_user.id, books_strings, reply_markup=inline_kb_full)
...
async def process_callback_kb1btn1(callback_query: types.CallbackQuery):
    ...
    await bot.answer_callback_query(callback_query.id, text='Я хочу страницу ' + str(action.payload.page_index) + ' из запроса под id: ' + str(action.payload.prepared_collection_id))
